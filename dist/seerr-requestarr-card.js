...
class SeerrRequestarrCard extends HTMLElement {
  ...
  async _loadDetail(media) {
    ...
    try {
      ...
      this._detailFull = media; // fallback to search data
    } finally {
      this._detailLoading = false;
    }
    this._paint();
  ...
  async _requestMedia(media) {
    ...
    try {
      ...
      // Refresh the full detail so status badge updates
      if (this._detailFull) { this._loadDetail(media); }
      this._loadRequests();
    } catch (e) {
  ...
  _browseHtml() {
    ...
    const grid  = this._browseData.length
      ? `<div class="poster-grid">${this._browseData.map(i => this._mediaCardHtml(i)).join("")}
         ${!this._browseDone ? `<div class="state-box" style="padding:16px;grid-column:1/-1">
           ${this._browseLoading
  ...
  // ── Detail HTML ───────────────────────────────────────────────────────────
  _detailHtml() {
    const m = this._detailFull || this._detail;
    if (!m) return "";

    const loading     = this._detailLoading && !this._detailFull;
      ...
      <div class="detail-outer">
        ...
        <div class="detail-scroll">
          ...
          <div class="detail-body">
            ...
            ${loading ? `<div class="state-box" style="padding:10px"><div class="spinner"></div></div>` : `
